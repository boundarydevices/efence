...
initialize(void)
{
	...
	if ( EF_ALIGNMENT == -1 ) {
		if ( (string = getenv("EF_ALIGNMENT")) != 0 )
			...
			if (__system_property_get("ef.alignment", prop)) {
				EF_ALIGNMENT = (size_t)atoi(prop);
			} else {
	...
	if ( EF_PROTECT_BELOW == -1 ) {
		if ( (string = getenv("EF_PROTECT_BELOW")) != 0 )
			...
			if (__system_property_get("ef.protect.below", prop)) {
				EF_PROTECT_BELOW = (atoi(prop) != 0);
			} else {
				EF_PROTECT_BELOW = 0;
			}
		}
	...
	if ( EF_PROTECT_FREE == -1 ) {
		if ( (string = getenv("EF_PROTECT_FREE")) != 0 )
			...
			if (__system_property_get("ef.protect.free", prop)) {
				EF_PROTECT_FREE = (atoi(prop) != 0);
			} else {
				EF_PROTECT_FREE = 0;
			}
		}
	...
	if ( EF_FILL == -1 ) {
		if ( (string = getenv("EF_FILL")) != 0)
			...
			if (__system_property_get("ef.fill", prop)) {
				EF_FILL = (unsigned char) atoi(prop);
			}
		}
#endif
	}
